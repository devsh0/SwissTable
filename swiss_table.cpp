...
struct SwissTable {
    ...
    // as SLOT_EMPTY need to have MSB set.
    ...
    void insert(const char* key, u32 value) {
        ...
        for (u64 i = 0; i < total_groups; i++) {
            ...
            slot = lookup_empty_slot_in_group(leader_vec, slot_leader);
            if (slot != SLOT_NONE) {
...
private:
    ...
    u64 lookup_slot(const char* key, u64 meta, u64 slot) {
        ...
        for (u64 i = 0; i < total_groups; i++) {
            ...
            if (lookup_empty_slot_in_group(v2, slot_leader) != SLOT_NONE) {
                return SLOT_NONE;
            }
            slot_leader += GROUP_SIZE;
            slot_leader %= total_slots;
    ...
    u64 lookup_empty_slot_in_group(__m256i leader_vec, u64 slot_leader) {
        auto res = _mm256_cmpeq_epi8(empty_slot_vec, leader_vec);
